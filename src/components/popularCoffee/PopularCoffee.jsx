  ...
  return (
    <div className="bg-[#F5F4F1] rounded-lg py-6 px-4 shadow-md flex flex-col md:flex-row items-center gap-4">
      ...
      <div className="flex md:flex-col gap-2 items-center">
        <Link
          to={`/detailsCoffee/${_id}`}
          className="btn btn-sm bg-[#3C393B] hover:bg-black text-white"
        >
          <FaEye />
        </Link>
        <Link
          to={`/updateCoffee/${_id}`}
          className="btn btn-sm bg-[#D2B48C] hover:bg-[#b5926f] text-white"
        >
          <FaEdit />
        </Link>
        <button
          onClick={() => handleDelete(_id)}
          className="btn btn-sm bg-[#EA4744] hover:bg-red-600 text-white"
        >
          <FaTrash />
        </button>
      </div>
